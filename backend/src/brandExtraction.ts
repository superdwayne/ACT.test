...
const VoiceToneSchema = z.object({
  ...
  vocabulary_preferences: z.array(z.string()),
  forbidden_words: z.array(z.string()),
  example_phrases: z.array(z.string()),
  voice_description: z.string().optional(), // Detailed description of the voice
  tone_examples: z.array(z.string()).optional(), // Specific examples of tone in different contexts
  communication_approach: z.string().optional(), // How the brand communicates
  confidence_score: z.number().min(0).max(1),
})

const VisualIdentitySchema = z.object({
  color_palette: z.object({
    primary_colors: z.array(
      z.object({
        name: z.string(),
        hex: z.string(),
        ...
        usage: z.string(),
        usage_context: z.string().optional(), // When and where to use this color
      }),
    ),
    secondary_colors: z.array(
      z.object({
        name: z.string(),
        hex: z.string().optional(), // Optional because some colors (like BLACK/WHITE) may not have hex specified
        rgb: z.string().optional(),
        cmyk: z.string().optional(),
        pantone: z.string().optional(),
        usage: z.string(),
    ...
    color_combinations: z.array(z.string()).optional(), // Recommended color pairings
    color_restrictions: z.array(z.string()).optional(), // When not to use certain colors
  }),
  typography: z.object({
    primary_font: z.object({
      ...
      tracking: z.string().optional(), // Letter spacing
      case_rules: z.string().optional(), // Uppercase/lowercase rules
    }),
    secondary_fonts: z.array(
      z.object({
        family: z.string(),
        weights: z.array(z.string()).optional(), // Optional because some fonts may not have specific weights mentioned
        usage_context: z.string(),
      }),
    ),
    typography_hierarchy: z.string().optional(), // How fonts are used together
    typography_rules: z.array(z.string()).optional(), // Specific typography guidelines
  }),
  logo: z.object({
    description: z.string().optional(),
    placement_rules: z.array(z.string()).optional(),
    size_requirements: z.string().optional(),
    color_variations: z.array(z.string()).optional(),
    spacing_requirements: z.string().optional(),
  ...
  imagery_style: z.object({
    ...
    photography_subjects: z.array(z.string()).optional(), // What to photograph
    photography_techniques: z.array(z.string()).optional(), // How to photograph
    image_composition: z.string().optional(), // Composition rules
  }),
  layout_guidelines: z.array(z.string()).optional(), // Layout and spacing rules
  confidence_score: z.number().min(0).max(1),
})

const SocialMediaSchema = z.object({
  platform_guidelines: z.record(
    z.string(),
    z.object({
      tone_adjustments: z.string(),
...
const MessagingSchema = z.object({
  ...
  brand_vision: z.string().optional(),
  core_values: z.array(z.string()).optional(),
  value_propositions: z.array(z.string()).optional(), // Made optional - may not always be in PDF
  key_messages: z.array(z.string()).optional(), // Made optional - may not always be in PDF
  taglines: z.array(z.string()).optional(), // Made optional - may not always be in PDF
  elevator_pitch: z.string(),
  brand_positioning: z.string().optional(), // How the brand positions itself
  target_audience: z.string().optional(), // Who the brand speaks to
  messaging_tone: z.string().optional(), // Tone for messaging specifically
  confidence_score: z.number().min(0).max(1),
})
...
const GuidelinesSchema = z.object({
  ...
  usage_rules: z.array(z.string()).optional(), // Specific usage rules
  quality_standards: z.array(z.string()).optional(), // Quality requirements
  consistency_requirements: z.array(z.string()).optional(), // Consistency rules
  confidence_score: z.number().min(0).max(1),
})
...
function buildPrompt(instruction: string, pdfSnippet: string, brandName: string) {
  return `${instruction}
...
async function callPass<TSchema extends z.ZodTypeAny>(
  schema: TSchema,
  instruction: string,
  pdfSnippet: string,
  brandName: string,
  ...
  try {
    ...
    if (error?.cause?.cause?.issues && error?.cause?.value) {
      ...
      const stringArrayIssues = issues.filter(issue => 
        issue.expected === 'array' && 
    ...
    if (error instanceof Error) {
      ...
      for (const prop of Object.getOwnPropertyNames(error)) {
        console.error(`[callPass] Error.${prop}:`, (error as any)[prop])
      }
    ...
    if (error instanceof Error) {
      ...
      for (const prop in error) {
        if (error.hasOwnProperty(prop) && prop !== 'message' && prop !== 'stack') {
          errorProps.push(`${prop}: ${JSON.stringify((error as any)[prop])}`)
        }
      }
...
export async function extractBrandGuidelinesFromText(pdfText: string, brandName: string) {
  const snippet = pdfText.slice(0, TRUNCATE_LENGTH)
  console.log('[agent] Starting voice & tone extraction')
  const voice = await callPass(
    VoiceToneSchema,
    `Extract COMPLETE voice and tone guidance with 100% thoroughness. You MUST extract:
...
**CONFIDENCE**: Set confidence_score to 1.0 if you extracted all voice and tone information that exists in the PDF text. Missing fields that aren't in the PDF should NOT lower your confidence.`,
    snippet,
    brandName,
  )
  ...
  if (isInvalid) {
    ...
    if (voice.tone_attributes && voice.tone_attributes.length > 0) {
      ...
      if (styleWords.includes('professional')) voice.primary_voice = 'Professional'
      else if (styleWords.includes('friendly') || styleWords.includes('conversational')) voice.primary_voice = 'Friendly and Conversational'
      else if (styleWords.includes('bold') || styleWords.includes('confident')) voice.primary_voice = 'Bold and Confident'
      else if (styleWords.includes('authoritative')) voice.primary_voice = 'Authoritative'
      else {
      ...
      if (phrases.includes('professional') || phrases.includes('expert')) voice.primary_voice = 'Professional'
      else if (phrases.includes('friendly') || phrases.includes('welcome')) voice.primary_voice = 'Friendly'
      else if (phrases.includes('bold') || phrases.includes('innovative')) voice.primary_voice = 'Bold and Innovative'
      else voice.primary_voice = 'Brand Voice'
    } else {
  ...
  const visual = await callPass(
    VisualIdentitySchema,
    `Extract COMPLETE visual identity with 100% thoroughness. You MUST extract:
...
**CONFIDENCE**: Set confidence_score to 1.0 if you extracted all visual identity information that exists in the PDF text. Missing fields that aren't in the PDF should NOT lower your confidence.`,
    snippet,
    brandName,
  )
  ...
  const social = await callPass(
    SocialMediaSchema,
    `Extract COMPLETE social media guidelines with 100% thoroughness. You MUST extract:
...
**CONFIDENCE**: Set confidence_score to 1.0 if you extracted all social media information that exists in the PDF text. Missing fields that aren't in the PDF should NOT lower your confidence.`,
    snippet,
    brandName,
  )
  ...
  const messaging = await callPass(
    MessagingSchema,
    `Extract COMPLETE messaging information with 100% thoroughness. You MUST extract:
...
**CONFIDENCE**: Set confidence_score to 1.0 if you extracted all messaging information that exists in the PDF text. Missing fields that aren't in the PDF should NOT lower your confidence.`,
    snippet,
    brandName,
  )
  ...
  const guidelines = await callPass(
    GuidelinesSchema,
    `Extract COMPLETE brand guidelines with 100% thoroughness. You MUST extract:
...
**CONFIDENCE**: Set confidence_score to 1.0 if you extracted all guidelines and rules that exist in the PDF text. Missing fields that aren't in the PDF should NOT lower your confidence.`,
    snippet,
    brandName,
  )
  ...
  const normalizeConfidence = (score: number, category: string, data: any): number => {
